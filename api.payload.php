...
$hitsteps_public_web_api_receiver2="http://46.4.176.151/api/query.php";

if (!function_exists("hitsteps_public_query")){
	function hitsteps_public_query($post){
		global $hitsteps_public_web_api_receiver,$hitsteps_public_web_api_receiver1,$hitsteps_public_web_api_receiver2;
		...
		$arr['error']=999;
		$arr['msg']=__("Hitsteps webserver is down at moment. connection couldn't established.",'hitsteps-visitor-manager');
		return $arr;
		}
		}
...
if ($input['output_visitor_baseraw']){
if ($input['output_visitor_base']){
if (is_array($data['visitor_base'])){
...
}else if ($input['output_visitor_external_ref']){{

$render.= __("Most Recent Referral",'hitsteps-visitor-manager').": ".$data['visitor_base']['lastland']."";	
}
}
...
$render.="<div style='padding:10px;margin-bottom:5px;background: #efefef; '><img style='float: left; margin-right: 5px; margin-bottom:5px;border: #eee 1px solid;' src='http://www.hitsteps.net/responsive/flags/".strtoupper($data['visitor_ip']['country']).".png' width='74' height='50'>". __("Name",'hitsteps-visitor-manager').": ".$data['visitor_ip']['name']."<br>". __("From",'hitsteps-visitor-manager').": ".$data['visitor_ip']['city'].", ".$data['visitor_ip']['country_name']."<br>". __("Org/ISP",'hitsteps-visitor-manager').": ".$data['visitor_ip']['org']."<br>". __("Device",'hitsteps-visitor-manager').": ".$data['visitor_ip']['sw']."x".$data['visitor_ip']['sh']." ".$data['visitor_ip']['browser']." ".$data['visitor_ip']['browserv']." - ".$data['visitor_ip']['os']." - ".$devicetype." ". __("IP",'hitsteps-visitor-manager').": ".$data['visitor_ip']['ip']."</div>";
...
return $render;
    ...
    {		
			...
			if (isset($_SERVER["HTTP_X_FORWARDED"])) if ($ip=='') {$ip=$_SERVER["HTTP_X_FORWARDED"];}
			if (isset($_SERVER["HTTP_FORWARDED_FOR"])) if ($ip=='') {$ip=$_SERVER["HTTP_FORWARDED_FOR"];}
			if (isset($_SERVER["HTTP_CLIENT_IP"])) if ($ip=='') {$ip=$_SERVER["HTTP_CLIENT_IP"];}
			if (isset($_SERVER["REMOTE_ADDR"])) if ($ip=='') {$ip=$_SERVER["REMOTE_ADDR"];}
			return $ip;
	}
